...
@click.option("--out_dir", default='./ensembles3')
@click.option(
    "--data_file",
...
@click.option("--taumax", default=0.0825)
def main(expt_dir, n_ensembles, out_dir, data_file, timesteps, save_file,
    ...
    for i, init_time in enumerate(ni):
        ...
        with torch.no_grad():
            ...
            st = time.time()
            expt1.recreate_model(mlp=True, device=device)
            expt1.create_forward_scenario(init_time,
                                          timesteps,
                                          data_file=data_file,
                                          save_file=save_file,
                                          device=device,
